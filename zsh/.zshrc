...
# Function to find files with fzf and copy the absolute path to clipboard
fzf-copy-path() {
  local file
  
  # Use find to list files and pipe to fzf for selection
  file=$(find . -type f | fzf --height 40% --reverse --preview 'cat {}')
  ...
  if [ -n "$file" ]; then
    ...
    if [ "$(uname)" = "Darwin" ]; then
      ...
      echo "Path copied to clipboard: $absolute_path"
      ...
      # Windows
      echo -n "$absolute_path" | clip
      echo "Path copied to clipboard: $absolute_path"
    else
      # Fallback if no clipboard command is available
      echo "Clipboard utility not found. Path: $absolute_path"
    fi
  fi
}

# Zsh key binding for Alt+P (Meta+p)
zle -N fzf-copy-path
